jQuery( function( $ ) {
    ...
    /* Pay Page Form */
    jQuery( '#mpesapay-payment-button' ).click( function() {
        return mpesaPayFormHandler();
    });

    jQuery( '#mpesapay_form form#order_review' ).submit( function() {
        return mpesaPayFormHandler();
    });

    function mpesaPayFormHandler() {

        if ( mpesapay_submit ) {
            mpesapay_submit = false;
            return true;
        }
