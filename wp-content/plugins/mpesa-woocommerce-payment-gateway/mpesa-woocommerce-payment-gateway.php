...
add_action( 'woocommerce_admin_order_data_after_billing_address', 'mpesa_editable_order_meta_billing' );	 	
function mpesa_editable_order_meta_billing( $order ){	 	 
	// get data functions here
	$payment = json_decode(get_post_meta( $order->id, 'Payment_Request',true));

	?>
	<div class="address">
		<p>
			<h3>Paymeny Request Results</h3><br>
			<?php 
				if(empty($payment)){
					echo 'No payment request for this Order.';
				}
				else{
					echo 'Merchant Request ID: '.$payment->MerchantRequestID.'<br>';
					echo 'Checkout ID: '.$payment->CheckoutRequestID.'<br>';
					echo 'Response Code: '.$payment->ResponseCode.'<br>';
					echo 'Response Description: '.$payment->ResponseDescription.'<br>';
					echo 'Customer Message: '.$payment->CustomerMessage.'<br>';
				}
...
function mpo_wc_mpesa_pay_init() {
	...
	class WC_Mpesa_Payonline_Gateway extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			//Actions
			add_action('woocommerce_api_callback', array($this, 'check_mpesa_response'));
			add_action( 'wp_enqueue_scripts', array( $this, 'payment_Mpesa' ) );
	    ...
	    **/
		...
		public function payment_Mpesa() {
			...
			'PhoneNumber' => '254728877615',
			'CallBackURL' => 'https://ancient-bird-83.localtunnel.me/projects/sparebox/sparebox/wc-api/Wc_Gateway_callbackClass',
			...
			update_post_meta( $order_id, 'Payment_Request', $curl_response, true );
			// update_post_meta( $order_id, 'Payment Details', $curl_response );
		...
		// Callback url
		// WC()-> api_request_url( 'wc_foobar_payment' );
		public function check_mpesa_response(){
			$postData = file_get_contents('php://input');
			add_post_meta( $order_id, 'Post Data', $postData, true );
		}
		...
		public function charge_token() {
			...
			if( isset( $_POST['wc_mpesapay_token'], $_POST['wc_mpesapay_order_id'] ) ) {
				...
				$body = array(
					...
					'CallBackURL'		=> 'https://tough-impala-62.localtunnel.me/mpesa/index.php',
					'AccountReference'	=> 'Titus',
